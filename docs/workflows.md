...
The registry ships five curated Package workflow namespaces with nineteen runnable names.
...
| `task/implement-plan-v2-template` | `task`             | Renders implement-plan-v2.workflow.mjs with one bounded agent-owned recovery per blocked step. |
