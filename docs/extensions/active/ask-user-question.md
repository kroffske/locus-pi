...
The entrypoint `extensions/ask-user-question/index.ts` calls `ask-tool.ts` to register the TypeBox-backed tool `ask` and the compatibility tool `askUserQuestion`. Their handlers, defined in `ask-tool.ts`, validate parameters through `_shared/validation.validateParams`, then invoke the OMP-compatible flow.
