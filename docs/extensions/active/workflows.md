...
| `plan`               | **Task to outcome-first accepted plan**: read-only throughout. A `scout` maps the repository, then a `planner`/`critic` loop defines one primary result before deriving steps: outcome type, consumer, form and location, required content or behavior, usability proof, and supporting evidence. The critic rejects plans whose steps can pass without producing that result, hidden meaning-changing assumptions, and both oversized steps and repeated mechanical one-item handoffs. The existing bounded round-cap handoff remains unchanged.                                                                                                                                                                                                                                                                                                             |
| `plan-implement`     | **Accepted plan to verified primary result**: one host-verified continuation artifact, pasted plan text, or one confined project-relative regular non-symlink UTF-8 file supplies the plan. Deterministic code requires exactly one supported, non-placeholder `## Outcome` contract, parses `### S<n>` blocks and `Depends on:` closure, restores selected plan order, and publishes `implementation-tasks.md`. Each step gets one writer and independent reviewer. The final checker returns structured selected-step and repository command statuses; deterministic validation refuses `complete` after any failed or unrun check, evidence gap, run-attributable unexpected change, non-ready primary result, or mismatch with the accepted outcome's exact deliverable name, kind, location, and proof. One bounded reconciliation may repair any terminal gap, including a missing result after all steps are done. The primary output is `workflow-summary.md`; `implementation-report.md` remains supporting per-step evidence. |
...
run-attributable unexpected change, missing primary result, or mismatch between
the accepted outcome's exact deliverable name, kind, location, and proof can be
projected as complete. The primary terminal document is
