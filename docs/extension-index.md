...
| `workflows`           |    84 | active | locus-specific / critical | Workflow runtime, `/workflows`, `workflow`, opt-in `/fusion` + `fusion`, and seven Package namespaces  | `extensions/workflows/index.ts`           | `extensions/workflows/manifest.json`           | `docs/extensions/active/workflows.md`           | none                        | `agent-runtime`, `host`, `model`, `operator`            | `@ast-grep/napi`, `@earendil-works/pi-coding-agent`, `@earendil-works/pi-tui`, `@sinclair/typebox` |
...
currently holds seven namespaces: six runnable roots, one group-only root, and
nine runnable children:
