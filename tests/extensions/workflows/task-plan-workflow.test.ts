...
describe("Package workflow: task/plan", () => {
  it("shares a group-only Package namespace with task/script and task/implement", () => {
    ...
    expect(buildWorkflowCatalogModel(root, root).groups).toContainEqual(
      expect.objectContaining({
        name: "task",
        source: "package",
        children: ["task/implement", "task/plan", "task/script"],
      }),
    );
  ...
  it("is a standard two-agent graph with no model pin, template, or script-owned planning logic", () => {
    ...
    expect(source).not.toContain("execute-template");
  ...
  it("hands reconnaissance text unchanged to the planner and publishes plan.md", async () => {
    ...
    expect(calls[1]?.prompt).toContain("Fully replace the planning files");
    expect(calls[1]?.prompt).toContain("`plan.md`");
    expect(calls[1]?.prompt).toContain("`step-1.md`, `step-2.md`");
    ...
    expect(calls[1]?.prompt).toMatch(/Do not create\s+`steps\.md`, `tasks\.md`/u);
    expect(calls[1]?.prompt).toContain("one complete flat block");
    expect(calls[1]?.prompt).toContain("use no nested structural headings");
    expect(calls[1]?.prompt).toMatch(/whose `S<n>`\s+matches the `<n>` in its file name/u);
    expect(calls[1]?.prompt).toMatch(/Delete any leftover `step-<n>\.md`/u);
    for (const field of [
      "Work unit:",
      "Boundary:",
      "Goal:",
    ...
    expect(calls[1]?.prompt).toMatch(/one frozen\s+`step-<n>\.md` catalog before execution/u);
    expect(calls[1]?.prompt).toContain("one top-level Task Implement run per step file");
    expect(calls[1]?.prompt).toMatch(/each run given\s+only the step id such as `S1`/u);
    expect(calls[1]?.prompt).toContain("the Package workflow `task/script`");
    ...
    expect(calls[1]?.prompt).toMatch(/the owner may edit those files first/u);
    expect(calls[1]?.prompt).toMatch(/files on disk stay the contract every later run reads/u);
    expect(calls[1]?.prompt).toMatch(/plan approval starts neither implementation nor workflow\s+authoring/u);
    expect(calls[1]?.prompt).toMatch(/renders no script/u);
    expect(calls[1]?.prompt).toMatch(/writes\s+Design, reviews it, and Builds matching source in the same turn/u);
    expect(calls[1]?.prompt).toContain("Do not\ninject `Design only`");
    expect(calls[1]?.prompt).toMatch(/optional reviewer after a generated step belongs to the bespoke\s+design/u);
    ...
    expect(result).toContain("/workflows run task/script");
  ...
  it("resumes after a planner failure without executing reconnaissance again", async () => {
    ...
    expect(existsSync(path.join(workspace, "execute.workflow.mjs"))).toBe(false);
