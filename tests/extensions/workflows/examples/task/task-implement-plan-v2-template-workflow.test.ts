...
const workflowPath = path.join(workflowDirectory, "implement-plan-v2-template.workflow.mjs");
const templateRelativePath = "resources/implement-plan-v2-template.prompt.md";
...
function renderImplementPlanV2(steps: Array<{ id: string; title: string; block: string }>): string {
...
describe("Package workflow: task/implement-plan-v2-template", () => {
  it("is one standard rendering agent with no planning child, parser, or model pin", () => {
    ...
    expect(source).toContain('promptFile("./resources/implement-plan-v2-template.prompt.md")');
  ...
  it("renders and publishes the approved plan without replanning or executing it", async () => {
    ...
    expect(record.calls).toEqual([
      { prompt: "RENDERED IMPLEMENT PLAN V2 TEMPLATE", label: "render-plan-v2", workspaceMode: "project" },
    ]);
    expect(record.published).toEqual(["implement-plan-v2.workflow.mjs"]);
  ...
  it("renders a standard sequential plan with one literal node per step", () => {
    const rendered = renderImplementPlanV2([
      {
        id: "S1",
    ...
    expect(rendered).toContain('name: "implement-plan-v2"');
    expect(rendered).toContain('profile: "standard"');
    // Per step: normal node, recovery, recovery review, and one retry; plus summary.
    expect(rendered.match(/await agent\(/gu)).toHaveLength(9);
    ...
    expect(rendered.match(/choice: \["recovered", "blocked"\]/gu)).toHaveLength(2);
    expect(rendered.match(/choice: \["retry", "blocked"\]/gu)).toHaveLength(2);
    ...
    expect(rendered).toContain('if (S1Recovery === "blocked")');
    expect(rendered).toContain('label: "S1-recovery"');
    expect(rendered).toContain('label: "S1-recovery-review"');
  ...
  it("turns an unrecovered blocked step into a workflow failure before the next literal node", async () => {
    const rendered = renderImplementPlanV2([
    ...
    await expect(
      module.default({
        ...
        agent: async (_prompt: string, options: { label: string }) => {
          ...
          if (options.label === "S1-recovery") return "blocked";
          return "completed";
        },
        publishPrimaryFile: () => ({ relativePath: "result.md" }),
      }),
    ).rejects.toThrow("Step S1 — Create the DAG is blocked; read history/S1.md and recovery/S1.md.");
    expect(blockedLabels).toEqual(["S1", "S1-recovery"]);
  ...
  it("lets agents recover, review, and retry one blocked step, then continues or fails", async () => {
    const rendered = renderImplementPlanV2([
      { id: "S1", title: "Create the DAG", block: "## S1 — Create the DAG\n\nGoal: add the DAG." },
    ...
    await module.default({
      ...
      agent: async (prompt: string, options: { label: string }) => {
        recoveredLabels.push(options.label);
        recoveredPrompts.push(prompt);
        if (options.label === "S2") return "blocked";
        if (options.label === "S2-recovery") return "recovered";
        if (options.label === "S2-recovery-review") return "retry";
        return options.label === "summary" ? "summary written" : "completed";
      },
      publishPrimaryFile: (relativePath: string) => {
        published.push(relativePath);
    ...
    expect(recoveredLabels).toEqual(["S1", "S2", "S2-recovery", "S2-recovery-review", "S2-retry", "summary"]);
    expect(published).toEqual(["result.md"]);
    expect(recoveredPrompts[2]).toContain("recovery agent");
    expect(recoveredPrompts[3]).toContain("independent recovery reviewer");
    expect(recoveredPrompts[3]).toContain("implement-plan-v2.workflow.mjs");
    expect(recoveredPrompts[3]).toContain("frozen approved block");
    expect(recoveredPrompts[3]).toMatch(/expected\s+evidence, not completed-step drift/u);
    expect(recoveredPrompts[4]).toContain("single retry implementation agent");
    expect(recoveredPrompts[4]).toMatch(/Read `plan\.md`, every\s+`step-<n>\.md`/u);
    expect(recoveredPrompts[4]).not.toContain("--- BEGIN EXACT STEP");

    // Recovery is accepted but the retry still blocks: exactly one round, then failure.
    ...
    await expect(
      module.default({
        ...
        agent: async (_prompt: string, options: { label: string }) => {
          ...
          if (options.label === "S1") return "blocked";
          if (options.label === "S1-recovery") return "recovered";
          if (options.label === "S1-recovery-review") return "retry";
    ...
    expect(stillBlockedLabels).toEqual(["S1", "S1-recovery", "S1-recovery-review", "S1-retry"]);
  });

  it("lets the recovery reviewer reject scope drift before retry", async () => {
    const rendered = renderImplementPlanV2([
      { id: "S1", title: "Create the DAG", block: "## S1 — Create the DAG\n\nGoal: add the DAG." },
      { id: "S2", title: "Remove cron", block: "## S2 — Remove cron\n\nGoal: remove cron." },
    ]);
    ...
    await expect(
      module.default({
        ...
        agent: async (_prompt: string, options: { label: string }) => {
          ...
          if (options.label === "S1") return "blocked";
          if (options.label === "S1-recovery") return "recovered";
          if (options.label === "S1-recovery-review") return "blocked";
          return "completed";
        },
        publishPrimaryFile: () => ({ relativePath: "result.md" }),
      }),
    ).rejects.toThrow("Step S1 — Create the DAG recovery was rejected; read recovery/S1.md.");
    expect(labels).toEqual(["S1", "S1-recovery", "S1-recovery-review"]);
  });

  it("keeps the generated plan out of registered directories and removes stale targets before rendering", () => {
    const template = readFileSync(templatePath, "utf8");

    expect(template).toContain("the Package workflow `task/implement-plan-v2-template`");
    ...
    expect(template).toContain("Do not delete or modify\n`implement-plan.workflow.mjs`");
    expect(template).toContain("leave the V2 target absent");
    ...
    // Bounded V2 recovery: agent decision, independent review, then one retry.
    expect(template).toContain("recovery agent");
    expect(template).toContain('choice: ["recovered", "blocked"]');
    expect(template).toContain("independent recovery reviewer");
    expect(template).toContain('choice: ["retry", "blocked"]');
    expect(template).toContain("Change only Verification and Done when");
    expect(template).toContain("single retry implementation agent");
