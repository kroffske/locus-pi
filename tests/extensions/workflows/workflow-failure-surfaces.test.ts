...
describe("workflow failure surfaces", () => {
  it("shows where a failed run broke in the live widget without a clipped repair request", () => {
    ...
    component.finish({
      ...
      runDir: ".locus/runtime/workflows/run-1",
    ...
    expect(text).toContain("answer: .locus/runtime/workflows/run-1/artifacts/answers/call-0003-inventory.md.md");
    expect(text).toContain("journal: .locus/runtime/workflows/run-1/journal.ndjson");
