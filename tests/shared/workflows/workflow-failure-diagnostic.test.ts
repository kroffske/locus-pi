...
describe("workflow failure diagnostic", () => {
  it("names the failing stage, owning script, and the answer that stage produced", () => {
    const diagnostic = buildWorkflowFailureDiagnostic({
      projectRoot: "/repo",
      runDir: "/repo/.locus/runtime/workflows/run-1",
      journalPath: "/repo/.locus/runtime/workflows/run-1/journal.ndjson",
    ...
    expect(diagnostic).toEqual({
      ...
      evidencePath: ".locus/runtime/workflows/run-1/artifacts/answers/call-0003-inventory.md.md",
      journalPath: ".locus/runtime/workflows/run-1/journal.ndjson",
      repairRequest:
        ...
        "Failing stage answer: .locus/runtime/workflows/run-1/artifacts/answers/call-0003-inventory.md.md. " +
        "Run journal: .locus/runtime/workflows/run-1/journal.ndjson.",
    });
  ...
  it("states what it cannot prove instead of guessing a stage, script, or answer", () => {
    const diagnostic = buildWorkflowFailureDiagnostic({
      projectRoot: "/repo",
      runDir: "/repo/.locus/runtime/workflows/run-2",
      journalPath: "/repo/.locus/runtime/workflows/run-2/journal.ndjson",
      journal: [],
    ...
    expect(diagnostic.repairRequest).toBe(
      "Diagnose this workflow: the workflow runtime failed — Pi SDK host: connection refused. " +
        "Run journal: .locus/runtime/workflows/run-2/journal.ndjson.",
    );
    expect(formatWorkflowFailureDiagnosticLines(diagnostic)).toEqual([
      "journal: .locus/runtime/workflows/run-2/journal.ndjson",
    ]);
