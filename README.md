...
`locus-pi` is a Pi extension package for agentic software-development workflows. It installs eleven default extensions, a bundled agent catalog, three agent skills, and a curated workflow registry with seventeen runnable workflow names.
...
| `task/plan`                   | Decomposed no-ask planning: scope, context, analyses, plan plus `step-<n>.md` files, reviews, one correction, verification. |
| `task/implement`              | Execute and verify one approved task step named by its step selector.                                                       |
| `task-via-script`             | Run the full `task/plan` pipeline, then render `implement.workflow.mjs` from the planned files.                             |
