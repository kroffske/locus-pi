...
export function getAgentWorkloadProof(sessionId: string | undefined, projectRoot?: string): AgentWorkloadProof | undefined {
  ...
  return { toolCallCount: merged.toolCallCount, toolResultCount: merged.toolResultCount, toolNames: [...merged.toolNames] };
}

export function writeAgentWorkloadProof(ctx: ExtensionContext, toolName: string): string {
  const projectRoot = getProjectRoot(ctx);
  const sessionId = getSessionId(ctx);
  const proof = mergeProof(getAgentWorkloadProof(sessionId, projectRoot), { toolCallCount: 1, toolResultCount: 1, toolNames: [toolName] })
  ...
  writeFileSync(filePath, `${JSON.stringify({ version: "locus.agent.workload-proof.v1", sessionId, ...proof }, null, 2)}\n`, "utf8");
  return filePath;
}
...
function readPersistedProof(projectRoot: string | undefined, sessionId: string): AgentWorkloadProof | undefined {
  ...
  try {
    ...
    return {
      ...
      toolNames: Array.isArray(parsed.toolNames) ? parsed.toolNames.filter((item): item is string => typeof item === "string") : [],
    };
  } catch {
    return undefined;
  }
...
function mergeProof(left: AgentWorkloadProof | undefined, right: AgentWorkloadProof | undefined): AgentWorkloadProof | undefined {
