  ...
  async function runAgentAttempt(
    ...
    if (checkSchema !== undefined && finalResult.ok && finalResult.status === "completed") {
      try {
        ...
        // agent_end and no record of why the run stopped. "Really ran" is also why it
        // carries the readback: the child answered before the validator was ever called.
        emit({
          ts: nowFn(),
          runId,
          kind: "error",
    ...
    try {
      ...
      // The artifact writer runs after the child returned, so this failure is the store's
      // and not the call's — the readback rides along for the same reason it does above.
      emit({
        ts: nowFn(),
        runId,
        kind: "error",
