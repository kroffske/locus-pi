...
// listing, queries and the summary projection. `isRecord`, `hasExactFields` and
// `isArtifactRef` come from there too, so the shapes a persisted binding is checked
// against are read with the same predicates the journal line is.
import {
  hasExactFields,
  isArtifactRef,
  isRecord,
