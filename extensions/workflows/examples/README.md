...
| [`task/plan.workflow.mjs`](./task/plan.workflow.mjs)                                                         | `standard` |   156 |              1 |            0 |       0 | npm package · public repository |
| [`task/implement.workflow.mjs`](./task/implement.workflow.mjs)                                               | `standard` |    66 |              0 |            0 |       0 | npm package · public repository |
...
| `task/plan`           | Task → repository map, plan, dynamic steps, and the execute script   | One reconnaissance agent writes `context.md`; one planning agent writes `plan.md` and complete `## S<n>` blocks in `steps.md`; one scripting agent renders `execute.workflow.mjs`.                              |
| `task/implement`      | One approved step → changes, checks, and history                     | One implementation agent receives one complete step, changes only that scope, verifies it, and writes `history/S<n>.md`.                                                                                        |
...
`task` is group-only and has no runnable root. Its `task/plan` and
`task/implement` children form a pair, but no workflow calls the other, and a
finished `task/plan` run starts nothing. The installed `locus-task-workflow` skill
...
without replacing unrelated todos, and start one top-level `task/implement` run
...
`task/plan` also renders `execute.workflow.mjs` into that same workspace from the
fixed template in `task/resources/execute-template.prompt.md`: one literal
...
| `task/plan`           | Two inline stage prompts; `resources/execute-template.prompt.md` (168 lines) holds the scripting charter and the fixed execute-script template it fills.                                                                            |
| `task/implement`      | One inline implementation prompt.                                                                                                                                                                                                   |
...
decision 6 of 2026-07-29 — see the `task/plan` / `task/implement` seam above.
...
| `task/plan`          | — (no shaped stage)                                                                                                                                                                                                                                                                            | —; planning text stays opaque to JavaScript                                                                                                            | —; blank input becomes an explicit blocking task                 |
| `task/implement`     | — (no shaped stage)                                                                                                                                                                                                                                                                            | —; one implementation result stays opaque to JavaScript                                                                                                | —; blank input becomes an explicit blocked step                  |
