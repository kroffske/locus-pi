...
export function buildWorkflowActionPrompt(intent: WorkflowBrowserIntent): string {
  ...
  return [`Request: ${request}`, "Skill: $pi-workflow-authoring", "", "Additional instructions:", ""].join("\n");
}
...
function workflowContractLines(projectRoot: string, workingDirectory: string): string[] {
  return [
    ...
    'agents: agent() returns exact non-empty child text; opts.agent selects a discovered catalog role, omitted agent uses role "default", opts.agentFile selects a source-relative workflow-local .agent.md file, and the selectors are mutually exclusive',
    "resources: promptFile() loads a source-relative .prompt.md file; local resource bytes are copied once into the run directory with SHA-256 evidence",
