 ...
 */
export function standardWorkflowSourceShapeErrors(source) {
    ...
    if (parseError !== undefined)
        return [`source parse failed: ${oneLineSourceShape(parseError.text())}`];
...
const STANDARD_DSL_METHODS = new Set(STANDARD_DSL_METHOD_NAMES);
const STANDARD_PUBLISHED_ARTIFACT_METHODS = new Set([
    "publishArtifact",
    "publishPrimaryArtifact",
    "publishPrimaryFile",
]);
...
function validateStandardTopLevel(root, errors) {
    let metaCount = 0;
    const runEntries = [];
    for (const statement of root.children()) {
        if (statement.kind() === "comment" || statement.kind() === "hash_bang_line" || statement.kind() === ";")
            continue;
    ...
    if (runEntries.length !== 1)
        errors.add("standard profile requires exactly one visible default run function");
...
function validateStandardDependencies(root, errors) {
    for (const statement of root.findAll({ rule: { kind: "import_statement" } })) {
        ...
        errors.add(specifier?.startsWith("node:") === true
    ...
    for (const statement of root.findAll({ rule: { kind: "export_statement" } })) {
        if (!statement.children().some((child) => child.kind() === "from"))
            continue;
        const specifier = staticStringValue(statement.children().find((child) => child.kind() === "string"));
        errors.add(specifier?.startsWith("node:") === true
...
function validateStandardOwnedPolicy(root, runEntry, errors) {
    ...
    for (const pair of root.findAll({ rule: { kind: "pair" } })) {
        ...
        if (key === "schema" || key === "validate")
            errors.add(`standard profile owns no raw ${key}`);
    }
...
function validateStandardIdentifierRoots(root, errors) {
    const bindings = standardLexicalBindings(root);
    const approvedGlobals = new Set(["Error"]);
    if (root.findAll({ rule: { kind: "this" } }).length > 0 ||
    ...
    for (const identifier of [
        ...
        if (approvedGlobals.has(identifier.text()))
            continue;
        const ancestorIds = new Set(identifier.ancestors().map((ancestor) => ancestor.id()));
        if (bindings.some((binding) => binding.name === identifier.text() && (binding.scopeId === root.id() || ancestorIds.has(binding.scopeId)))) {
            continue;
        }
        errors.add("standard profile reads values only from declared lexical bindings and approved language roots");
    }
}
function standardLexicalBindings(root) {
    const bindings = [];
    const add = (names, scope) => {
        for (const name of names)
            bindings.push({ name, scopeId: scope.id() });
    };
...
function standardLexicalOwner(node, root) {
    ...
    if (loop !== undefined)
        return loop;
    return (node.ancestors().find((ancestor) => ancestor.kind() === "statement_block" || ancestor.kind() === "switch_body") ??
        root);
}
function validateStandardExpressions(root, protectedBindings, dslBindings, bindingModel, errors) {
    ...
    for (const expression of root.findAll({ rule: { kind: "new_expression" } })) {
        ...
        if (containsNonAuthorKnownValue(expression.field("arguments") ?? undefined, bindingModel.provenance, dslBindings, bindingModel.literalShadows)) {
            errors.add("standard profile constructs Error only from author-known or literal values");
        }
    }
}
function isOwnedForLoopCounterMutation(expression, protectedBindings) {
    if (expression.kind() === "assignment_expression")
        return false;
    const loop = expression.ancestors().find((ancestor) => ancestor.kind() === "for_statement");
    if (loop?.field("increment")?.id() !== expression.id())
        return false;
    const target = expression.field("left") ?? expression.field("argument");
    if (target?.kind() !== "identifier" || protectedBindings.has(target.text()))
        return false;
    ...
    const counterDeclaration = initializer
        ...
        .find((declaration) => declaration.field("name")?.kind() === "identifier" && declaration.field("name")?.text() === target.text());
    if (counterDeclaration?.field("value")?.kind() !== "number")
        return false;
    if (expression.kind() === "update_expression")
        return true;
    return (expression.kind() === "augmented_assignment_expression" &&
        ["+=", "-="].includes(expression.field("operator")?.text() ?? "") &&
        expression.field("right")?.kind() === "number");
}
function validateStandardCalls(root, runEntry, dslBindings, collectionBindings, errors) {
    validateStandardBindingShadows(root, runEntry, dslBindings, collectionBindings, errors);
    for (const call of root.findAll({ rule: { kind: "call_expression" } })) {
        const callee = unwrapStandardParentheses(callCallee(call));
        if (callee == null || callee.kind() === "import")
            continue;
...
function standardDslBindings(runEntry) {
    ...
    if (firstParameter?.kind() === "object_pattern")
        addStandardDslBindings(bindings, firstParameter);
...
function addStandardDslBindings(bindings, pattern) {
    for (const child of pattern.children()) {
        if (child.kind() === "shorthand_property_identifier_pattern" && STANDARD_DSL_METHODS.has(child.text())) {
            ...
            if (key !== undefined && key === value && STANDARD_DSL_METHODS.has(key))
                bindings.add(key);
        }
    }
}
function directStandardDslCall(callee, bindings) {
    ...
    return property !== undefined && STANDARD_DSL_METHODS.has(property) ? property : undefined;
}
function standardCollectionBindings(root, runEntry, dslBindings) {
    const names = new Set();
    ...
    const bodyDeclarations = body
        ?.children()
        .flatMap((statement) => statement.kind() === "lexical_declaration"
    ...
    for (const declaration of declarations) {
        ...
        if (name?.kind() !== "identifier" || names.has(name.text()))
            continue;
...
function validateStandardBindingShadows(root, runEntry, dslBindings, collectionBindings, errors) {
    if (runEntry === undefined)
        return;
    ...
    for (const name of boundStandardNames(runParameters)) {
        if (name === "Error")
            errors.add("standard profile does not shadow the global Error constructor");
    }
    for (const callback of [
    ...
    for (const declaration of root.findAll({ rule: { kind: "variable_declarator" } })) {
        ...
        const trustedDslDestructure = ownerBlock?.id() === runBody?.id() &&
            dslBindings.has("dsl") &&
            declaration.field("value")?.text() === "dsl" &&
            declaration.field("name")?.kind() === "object_pattern" &&
            names.length > 0 &&
            names.every((name) => name !== "dsl" && dslBindings.has(name) && STANDARD_DSL_METHODS.has(name));
        const trustedCollection = collectionBindings.ownerIds.has(declaration.id()) &&
...
function boundStandardNames(pattern) {
...
function standardFunctionParameters(callable) {
    return callable.field("parameters") ?? callable.field("parameter") ?? undefined;
}
function standardFunctionParameterNodes(parameters) {
    ...
    return parameters
        .children()
        .filter((child) => ["array_pattern", "assignment_pattern", "identifier", "object_pattern", "rest_pattern"].includes(String(child.kind())));
}
...
function standardBindingModel(root, runEntry, dslBindings, errors) {
    const collections = standardCollectionBindings(root, runEntry, dslBindings);
    if (runEntry === undefined)
        return { collections, literalShadows: [], provenance: new Map() };
    const values = standardValueProvenance(root, runEntry, dslBindings, collections, errors);
    return { collections, ...values };
}
function validateStandardValueUses(root, runEntry, dslBindings, bindingModel, errors) {
    if (runEntry === undefined)
        return;
    ...
    for (const access of [
        ...
        const owner = standardExpressionProvenance(access.field("object") ?? undefined, provenance, dslBindings, literalShadows);
        if (owner === undefined)
            continue;
        if (owner.kind === "opaque-value" ||
            owner.kind === "map-item" ||
            owner.kind === "runtime-value" ||
            owner.kind === "void-value" ||
            owner.kind === "unclassified-dsl-value") {
            if (!isInsideBoundaryInputDefault(access)) {
                errors.add("standard profile does not inspect properties of opaque semantic, model, file, host, or runtime values");
            }
            continue;
        }
        if (owner.kind === "runtime-status") {
            if (access.kind() === "member_expression" && access.field("property")?.text() === "status")
                continue;
        ...
        if (property === "length" || property === "map")
            continue;
        if (property === "join" && isInsideApprovedOpaqueSink(access, dslBindings))
            continue;
    ...
    for (const expression of [
        ...
        if (isInsideBoundaryInputDefault(expression))
            continue;
    ...
    for (const template of root.findAll({ rule: { kind: "template_string" } })) {
        if (containsOpaqueValue(template, provenance, dslBindings, literalShadows) &&
    ...
    for (const identifier of [
        ...
        if (isInsideLiteralShadow(identifier, literalShadows))
            continue;
        const value = provenance.get(identifier.text());
        if (value === undefined || ["known-collection", "known-value", "runtime-control"].includes(value.kind))
            continue;
        if (isStandardBindingOccurrence(identifier) || isDirectProvenanceAlias(identifier))
            continue;
        ...
        if (isInsideBoundaryInputDefault(identifier) || isInsideApprovedOpaqueSink(identifier, dslBindings))
            continue;
        if (value.kind === "opaque-list" && isOpaqueListStructuralUse(identifier))
            continue;
        if (value.kind === "runtime-status" && isRuntimeStatusIdentityUse(identifier))
            continue;
        if (isWholeValueReturnUse(identifier))
            continue;
        if (isPublishedArtifactContinuationUse(identifier, value, dslBindings))
            continue;
        if (isPublishedArtifactHandoffDetailUse(identifier, value, dslBindings))
            continue;
        if (isUnchangedScheduledValueUse(identifier, value, dslBindings))
            continue;
        errors.add("standard profile forwards opaque semantic, model, file, host, and runtime values only as whole values");
    }
}
function standardValueProvenance(root, runEntry, dslBindings, collections, errors) {
    const provenance = new Map();
    const owners = new Map();
    const duplicateNames = new Set();
    const reserve = (name, value, ownerId) => {
    ...
    for (const loop of runEntry.findAll({ rule: { kind: "for_in_statement" } })) {
        ...
        if (list?.kind !== "opaque-list" && list?.kind !== "known-collection")
            continue;
    ...
    for (const declaration of root.findAll({ rule: { kind: "variable_declarator" } })) {
        ...
        if (value === undefined && scope !== undefined)
            literalShadows.push({ name: name.text(), scopeId: scope.id() });
    }
    return { literalShadows, provenance };
}
function classifyStandardCallbackParameters(root, runEntry, provenance, dslBindings, reserve, errors) {
    const callbacks = [
        ...root.findAll({ rule: { kind: "arrow_function" } }),
        ...root.findAll({ rule: { kind: "function_expression" } }),
    ];
    for (const callback of callbacks) {
        ...
        const ownerCall = callback.ancestors().find((ancestor) => {
            if (ancestor.kind() !== "call_expression")
                return false;
            return standardCallArguments(ancestor).some((argument) => unwrapStandardParentheses(argument)?.id() === callback.id());
        });
        ...
        const callbackArgumentIndex = ownerArguments.findIndex((argument) => unwrapStandardParentheses(argument)?.id() === callback.id());
        if (method === "pipeline" && callbackArgumentIndex > 0) {
            classifyKnownStandardCallbackParameters(parameters, ["opaque-value", "runtime-control"], reserve, errors, "pipeline stage");
            continue;
        }
        errors.add("standard profile classifies every value-bearing callback parameter");
    }
}
function classifyKnownStandardCallbackParameters(parameters, kinds, reserve, errors, owner) {
...
function collectStandardDeclarationProvenance(root, provenance, dslBindings, errors, reserve) {
    for (const declaration of root.findAll({ rule: { kind: "variable_declarator" } })) {
        const value = standardExpressionProvenance(declaration.field("value") ?? undefined, provenance, dslBindings);
        if (value === undefined)
            continue;
...
function standardExpressionProvenance(node, provenance, dslBindings, literalShadows = []) {
    const value = unwrapStandardValue(node);
    if (value === undefined)
        return undefined;
    if (value.kind() === "identifier" || value.kind() === "shorthand_property_identifier") {
        if (isInsideLiteralShadow(value, literalShadows))
            return undefined;
    ...
    if (value.kind() === "member_expression" || value.kind() === "subscript_expression") {
        const owner = standardExpressionProvenance(value.field("object") ?? undefined, provenance, dslBindings, literalShadows);
        if (owner?.kind === "runtime-status" &&
            value.kind() === "member_expression" &&
            value.field("property")?.text() === "status") {
            return { kind: "runtime-control" };
        }
        if (owner?.kind !== "opaque-list")
            return undefined;
    ...
    const method = directStandardDslCall(callee, dslBindings);
    if (method !== undefined)
        return standardDslCallProvenance(method, value);
    if (callee.kind() === "member_expression" && callee.field("property")?.text() === "map") {
        const receiver = standardExpressionProvenance(callee.field("object") ?? undefined, provenance, dslBindings, literalShadows);
        if (receiver?.kind === "known-collection")
            return { kind: "known-collection" };
        if (receiver?.kind === "opaque-list")
            return { kind: "opaque-list" };
    }
    return undefined;
}
function standardDslCallProvenance(method, call) {
    const category = STANDARD_DSL_RETURN_CATEGORIES[method];
    if (category === undefined)
        return { kind: "unclassified-dsl-value", sourceMethod: method };
    if (category !== "agent-dependent")
        return { kind: category, sourceMethod: method };
    const optionKeys = new Set(standardCallArguments(call)[1]
        ?.children()
        .filter((child) => child.kind() === "pair")
        .map((pair) => staticObjectKey(pair.field("key"))) ?? []);
    if (optionKeys.has("choice"))
        return { kind: "runtime-control", sourceMethod: method };
    if (optionKeys.has("handoffs"))
        return { kind: "opaque-list", sourceMethod: method };
    return { kind: "opaque-value", sourceMethod: method };
}
function standardCompositeContainsRuntimeValue(composite, provenance, dslBindings, literalShadows) {
...
function standardCompositeValueExpressions(composite) {
    const values = [];
...
function containsOpaqueValue(node, provenance, dslBindings, literalShadows = []) {
    const candidates = [
        node,
        ...[
            "call_expression",
    ...
    return candidates.some((candidate) => {
        ...
        return (value?.kind === "opaque-value" ||
            value?.kind === "map-item" ||
...
function containsNonAuthorKnownValue(node, provenance, dslBindings, literalShadows) {
    if (node === undefined)
        return false;
    const candidates = [
        node,
...
function containsOpaqueIndexValue(node, provenance, dslBindings, literalShadows) {
    if (node === undefined)
        return false;
    const candidates = [
        node,
...
function standardLoopBindingNames(left) {
    if (left?.kind() !== "lexical_declaration")
        return boundStandardNames(left);
    return left
        .children()
        .filter((child) => child.kind() === "variable_declarator")
        .flatMap((declaration) => boundStandardNames(declaration.field("name") ?? undefined));
}
function standardCallArguments(call) {
    return (call
        .children()
...
function isInsideBoundaryInputDefault(node) {
    const expression = node.kind() === "ternary_expression"
        ? node
...
function isInsideApprovedOpaqueSink(node, dslBindings) {
    for (const call of node.ancestors().filter((ancestor) => ancestor.kind() === "call_expression")) {
        ...
        if (argumentIndex < 0)
            continue;
        const approved = ((method === "agent" || method === "log") && argumentIndex === 0) ||
...
function isWholeValueOpaqueSinkPath(node, argument) {
    if (argument === undefined)
        return false;
    let current = node;
    while (current.id() !== argument.id()) {
        const parent = current.parent();
        if (parent === null || !nodeWithinStandardNode(parent, argument))
            return false;
        if (parent.kind() === "binary_expression") {
            if (parent.field("operator")?.text() !== "+")
                return false;
        }
        else if (!["parenthesized_expression", "template_string", "template_substitution"].includes(String(parent.kind()))) {
            return false;
        }
        current = parent;
    }
...
function isInsideLiteralShadow(identifier, shadows) {
    const ancestorIds = new Set(identifier.ancestors().map((ancestor) => ancestor.id()));
    return shadows.some((shadow) => shadow.name === identifier.text() && ancestorIds.has(shadow.scopeId));
}
function isStandardBindingOccurrence(identifier) {
    for (const declaration of identifier.ancestors().filter((ancestor) => ancestor.kind() === "variable_declarator")) {
        if (nodeWithinStandardNode(identifier, declaration.field("name") ?? undefined))
            return true;
    }
    for (const callable of identifier
        .ancestors()
        .filter((ancestor) => ["arrow_function", "function_declaration", "function_expression"].includes(String(ancestor.kind())))) {
        if (nodeWithinStandardNode(identifier, standardFunctionParameters(callable)))
            return true;
    }
    for (const loop of identifier.ancestors().filter((ancestor) => ancestor.kind() === "for_in_statement")) {
        if (nodeWithinStandardNode(identifier, loop.field("left") ?? undefined))
            return true;
    }
    return false;
}
function isDirectProvenanceAlias(identifier) {
...
function isOpaqueListStructuralUse(identifier) {
...
function isRuntimeStatusIdentityUse(identifier) {
    const parent = identifier.parent();
    return (parent?.kind() === "member_expression" &&
        parent.field("object")?.id() === identifier.id() &&
        parent.field("property")?.text() === "status");
}
function isWholeValueReturnUse(identifier) {
    for (const ancestor of identifier.ancestors()) {
        if (ancestor.kind() === "return_statement")
            return true;
        if (![
            "array",
            "await_expression",
...
function isPublishedArtifactContinuationUse(identifier, provenance, dslBindings) {
    if (provenance.kind !== "runtime-value" ||
        provenance.sourceMethod === undefined ||
        !STANDARD_PUBLISHED_ARTIFACT_METHODS.has(provenance.sourceMethod)) {
        return false;
    }
    let element = identifier;
    while (element.parent()?.kind() === "parenthesized_expression")
        element = element.parent();
    const refs = element.parent();
    if (refs?.kind() !== "array")
        return false;
    const refsPair = refs.parent();
    if (refsPair?.kind() !== "pair" ||
    ...
    if (call === undefined || standardCallArguments(call)[0]?.id() !== declaration.id())
        return false;
    const callee = unwrapStandardParentheses(callCallee(call));
    return callee !== undefined && directStandardDslCall(callee, dslBindings) === "awaitOperator";
}
function isPublishedArtifactHandoffDetailUse(identifier, provenance, dslBindings) {
    if (provenance.kind !== "runtime-value" ||
        provenance.sourceMethod === undefined ||
        !STANDARD_PUBLISHED_ARTIFACT_METHODS.has(provenance.sourceMethod)) {
        return false;
...
function isUnchangedScheduledValueUse(identifier, provenance, dslBindings) {
    ...
    if (propertyName === "outputDir") {
        if (provenance.kind !== "runtime-value" || provenance.sourceMethod !== "outputDir")
            return false;
    }
    ...
    if (container === undefined || !nodeWithinStandardNode(identifier, value ?? undefined))
        return false;
    if (pair !== undefined) {
        for (const ancestor of identifier.ancestors()) {
            if (ancestor.id() === container.id())
                break;
            if (!["array", "parenthesized_expression"].includes(String(ancestor.kind())))
                return false;
        }
    }
...
function isDiscardedStandardCall(call) {
    let current = call;
...
function isWholeValueCallArgument(identifier, methods, index) {
...
function nodeWithinStandardNode(node, container) {
    return (container !== undefined &&
        (node.id() === container.id() || node.ancestors().some((item) => item.id() === container.id())));
}
function isStandardCollectionExpression(node, bindings, dslBindings) {
    const value = unwrapStandardValue(node);
    if (value?.kind() === "array")
        return true;
    if (value?.kind() === "identifier")
        return bindings.has(value.text());
    if (value?.kind() !== "call_expression")
        return false;
    const callee = unwrapStandardParentheses(callCallee(value));
    if (callee === undefined)
    ...
    if (dslMethod !== undefined)
        return STANDARD_COLLECTION_DSL_METHODS.has(dslMethod);
...
function isVisibleCollectionCall(call, bindings, dslBindings) {
    const callee = unwrapStandardParentheses(callCallee(call));
    if (callee?.kind() !== "member_expression")
        return false;
    const method = callee.field("property")?.text();
    if (!isKnownCollectionReceiver(callee.field("object") ?? undefined, bindings, dslBindings))
        return false;
    if (method === "map") {
        const args = call.children().find((child) => child.kind() === "arguments");
        return (args?.children().some((child) => child.kind() === "arrow_function" || child.kind() === "function_expression") ===
            true);
    }
    if (method !== "join")
        return false;
...
function isBoundaryInputDefaultExpression(value) {
    return /^typeof\s+input\s*===\s*["']string["']\s*&&\s*input\.trim\(\)\s*\?\s*input\.trim\(\)\s*:\s*["'][^"']+["']$/u.test(value.text());
}
function containsStandardEdgeCall(node) {
    return node
        .findAll({ rule: { kind: "call_expression" } })
        .some((call) => [...STANDARD_EDGE_METHODS].some((edge) => isDirectStandardEdgeCall(call, edge)));
}
function isVisibleInlineEdgeCallback(callback) {
...
function isLiteralConstDeclaration(statement) {
    if (!statement.children().some((child) => child.kind() === "const"))
        return false;
    const declarations = statement.children().filter((child) => child.kind() === "variable_declarator");
    return (declarations.length > 0 && declarations.every((declaration) => isStaticAuthoringLiteral(declaration.field("value"))));
}
function isExactLiteralMetaExport(statement, meta) {
    const declaration = statement.children().find((child) => child.kind() === "lexical_declaration");
    if (declaration === undefined || !declaration.children().some((child) => child.kind() === "const"))
        return false;
...
function staticMetaProfile(meta) {
    const profile = meta
        .children()
        .find((child) => child.kind() === "pair" && staticObjectKey(child.field("key")) === "profile");
    return staticStringValue(profile?.field("value"));
}
function isStaticAuthoringLiteral(node) {
    if (node == null)
        return false;
    if (["false", "null", "number", "regex", "string", "true", "undefined"].includes(String(node.kind())))
        return true;
    if (node.kind() === "template_string")
        return staticStringValue(node) !== undefined;
    if (node.kind() !== "array" && node.kind() !== "object")
        return false;
    return node.children().every((child) => {
        if (isStructuralLiteralNode(child))
            return true;
        if (child.kind() === "pair")
...
function isStructuralStatementNode(node) {
    return node.kind() === "{" || node.kind() === "}" || node.kind() === "comment" || node.kind() === ";";
}
function exportedMetaObject(statement) {
    const declaration = statement.children().find((child) => child.kind() === "lexical_declaration");
...
function staticStringValue(node) {
    if (node == null || (node.kind() !== "string" && node.kind() !== "template_string"))
        return undefined;
    let value = "";
    for (const child of node.children()) {
        if (child.kind() === "string_fragment")
            ...
            value += decodeEscapeSequence(child.text());
        else if (child.kind() === "template_substitution")
            return undefined;
    }
    return value;
}
function staticObjectKey(node) {
    if (node == null || node.kind() === "computed_property_name")
        return undefined;
    if (node.kind() === "string")
        return staticStringValue(node);
    return node.text();
}
function decodeEscapeSequence(value) {
    const body = value.slice(1);
    const fixed = { n: "\n", r: "\r", t: "\t", b: "\b", f: "\f", v: "\v", 0: "\0" };
    if (fixed[body] !== undefined)
        return fixed[body];
...
function isStructuralLiteralNode(node) {
    const kind = node.kind();
    return kind === "{" || kind === "}" || kind === "[" || kind === "]" || kind === "," || kind === "comment";
}
function oneLineSourceShape(value) {
